...
代码包名为 `agent_eval`，安装后的命令为 `agent-eval`。更新开发副本后重新执行 `python -m pip install -e .`；旧论文评测入口及执行器已移除。已有 Terminal-Bench 快照冻结了旧代码路径与哈希，升级后应重新导入，创建 `1.2.3-dev.1` 快照供新任务使用。旧评测记录保留，不能直接复用旧快照运行已改名的代码。
...
| 环境变量 | 用途 |
| --- | --- |
| `DEEPSEEK_API_KEY` | Terminal-Bench 容器中的 DSH 执行模型 |
...
凭据在进程环境中提供，不填进 Snapshot JSON 或代码。Harbor 运行器也兼容已有 DSH 凭据库中的 `DEEPSEEK_API_KEY`，不会复制整个用户目录。
