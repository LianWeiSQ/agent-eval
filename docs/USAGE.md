...
默认 Supervisor 为 `LLM Supervisor Agent@1.3.3`，初始配置使用 32768 输出 token 预算。监督的 `pass` 表示任务完成；同意失败任务被判零分应报告 `fail`。与 Trial 未通过结果冲突的 `pass` 会保留原回复并转入人工复核。实际是否可调用，仍取决于接口地址、权限、网络和模型响应。切换配置应创建新快照。

重启后可在 PowerShell 核对服务实际加载的规则：

```powershell
(Invoke-RestMethod 'http://127.0.0.1:8765/health').data
```

当前应包含 `supervision_contract_version: task-completion-v2`。这只验证加载版本，不会调用模型，也不能替代真实监督接口校验。
...
4. 等当前批次全部执行结束后，按需对样本运行独立监督；选择监督快照不会自动触发整批监督。
...
监督提示 SSL/TLS、连接中断或 `supervisor_provider_unavailable` 时，先查看 API 错误和审计记录，核对是否已经生成有效监督记录。此类故障不表示模型判错；缺少响应时也不能断言没有产生费用。恢复后应按实验预算决定是否重试。
